...
cap log close
macro drop hr
log using ./logs/cox_2017_case.log, replace t
...
stcox i.case i.sex i.ethnicity i.imd i.urban i.bmi i.smoking age1 age2 age3, vce(cluster practice_id) strata(set_id)
if _rc==0{
estimates
...
stcox i.case i.sex i.ethnicity i.imd i.urban i.bmi i.cardiovascular i.diabetes i.hypertension i.immunosuppressed i.non_haem_cancer i.smoking age1 age2 age3, vce(cluster practice_id) strata(set_id)		
if _rc==0{
estimates
...
esttab model1 model2 model3 model4 using "./output/estout_cox_2017_case_`outcome'.txt", b(a2) ci(2) label wide compress eform ///
	title ("`outcome'") ///
	varlabels(`e(labels)') ///
	stats(N_sub) ///
	append 
