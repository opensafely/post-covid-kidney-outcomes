...
cap log close
macro drop hr
log using ./logs/covariates_egfr_half_severity_2020.log, replace t

use ./output/analysis_complete_2020.dta, clear
replace covid_severity=2 if covid_severity==3

cap file close tablecontent
file open tablecontent using ./output/covariates_egfr_half_severity_2020.csv, write text replace
...
file write tablecontent ("`label_`j''")
forvalues i=1/2 {
qui safecount if covid_vax==`j' & covid_severity==`i' & _d==1
local events_`j'_`i' = round(r(N),5)
}
forvalues i=1/2{
if `events_`j'_`i''>5 & `events_`j'_`i''!=. {
