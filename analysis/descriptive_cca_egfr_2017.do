...
cap log close
macro drop hr
log using ./logs/descriptive_cca_egfr_2017.log, replace t

cap file close tablecontent
file open tablecontent using ./output/descriptive_cca_egfr_2017.csv, write text replace
file write tablecontent _tab ("COVID-19 cohort (n(%))") _tab ("Matched historical cohort (n(%))") _n
...
local cases_`vax'_pc = (`cases_`vax''/`cases_2017')*100
qui safecount if covid_vax==`vax' & case==0
local controls_`vax' = round(r(N),5)
local controls_`vax'_pc = (`controls_`vax''/`controls_2017')*100
...
local cases_`wave'_pc = (`cases_`wave''/`cases_2017')*100
qui safecount if wave==`wave' & case==0
local controls_`wave' = round(r(N),5)
local controls_`wave'_pc = (`controls_`wave''/`controls_2017')*100
