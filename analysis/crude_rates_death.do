...
cap log close
macro drop hr
log using ./logs/crude_rates_death.log, replace t

cap file close tablecontent
file open tablecontent using ./output/crude_rates_death.csv, write text replace
...
use ./output/analysis_`x'.dta
stset exit_date_death, fail(death_date) origin(index_date_death) id(unique) scale(365.25)
qui safecount if ethnicity1==`ethnicity1' & case==1 & death==1 & _st==1
local cases_events = round(r(N),5)
local cases_rate : di %3.2f (`cases_events' * `cases_multip_`x'')
local cases_ef = exp(1.96/(sqrt(`cases_events')))
local cases_ul = `cases_rate' * `cases_ef'
local cases_ll = `cases_rate' / `cases_ef'
qui safecount if ethnicity1==`ethnicity1' & case==0 & death==1 & _st==1
local controls_events = round(r(N),5)
local controls_rate : di %3.2f (`controls_events' * `controls_multip_`x'')
local controls_ef = exp(1.96/(sqrt(`controls_events')))
local controls_ul = `controls_rate' * `controls_ef'
