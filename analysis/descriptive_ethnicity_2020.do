...
cap log close
macro drop hr
log using ./logs/descriptive_ethnicity_2020.log, replace t

cap file close tablecontent
file open tablecontent using ./output/descriptive_ethnicity_2020.csv, write text replace
...
forvalues i=1/6 {
qui safecount if wave==`wave' & case==1 & ethnicity1==`i'
local cases_`wave'_`i' = round(r(N),5)
local cases_`wave'_pc_`i' = (`cases_`wave'_`i''/`cases_`i'')*100
qui safecount if wave==`wave' & case==0 & ethnicity1==`i'
