...
cap log close
macro drop hr
log using ./logs/cox_2017_covid_aki.log, replace t

cap file close tablecontent
file open tablecontent using ./output/cox_2017_covid_aki.txt, write text replace
file write tablecontent ("Kidney outcomes by COVID-19 AKI compared to matched historical general population") _n
...
stcox i.covid_aki i.sex i.ethnicity i.imd i.urban i.bmi i.smoking age1 age2 age3, vce(cluster practice_id) strata(set_id)
if _rc==0{
estimates
...
stcox i.covid_aki i.sex i.ethnicity i.imd i.urban i.bmi i.cardiovascular i.diabetes i.hypertension i.immunosuppressed i.non_haem_cancer i.smoking age1 age2 age3, vce(cluster practice_id) strata(set_id)		
if _rc==0{
estimates
...
local lab3: label covid_aki 3
	...
	qui safecount if covid_aki== 1 & `outcome'==1
	local event = r(N)
...
forvalues severity=1/3 {
	qui safecount if covid_aki==`severity'
	local denominator = r(N)
