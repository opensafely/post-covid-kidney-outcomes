...
file open tablecontent using ./output/descriptive_hospitalised.csv, write text replace
file write tablecontent _tab ("Hospitalised COVID-19 cohort (n(%))") _tab ("Hospitalised pneumonia cohort (n(%))") _n
...
file write tablecontent %9.0f ("`cases_hospitalised'") _tab %9.0f ("`controls_hospitalised'") _n
...
local cases_`age'_pc = (`cases_`age''/`cases_hospitalised')*100
qui safecount if agegroup==`age' & case==0
local controls_`age' = round(r(N),5)
local controls_`age'_pc = (`controls_`age''/`controls_hospitalised')*100
...
local cases_`sex'_pc = (`cases_`sex''/`cases_hospitalised')*100
qui safecount if sex==`sex' & case==0
local controls_`sex' = round(r(N),5)
local controls_`sex'_pc = (`controls_`sex''/`controls_hospitalised')*100
...
local cases_`imd'_pc = (`cases_`imd''/`cases_hospitalised')*100
qui safecount if imd==`imd' & case==0
local controls_`imd' = round(r(N),5)
local controls_`imd'_pc = (`controls_`imd''/`controls_hospitalised')*100
...
local cases_`ethnicity1'_pc = (`cases_`ethnicity1''/`cases_hospitalised')*100
qui safecount if ethnicity1==`ethnicity1' & case==0
local controls_`ethnicity1' = round(r(N),5)
local controls_`ethnicity1'_pc = (`controls_`ethnicity1''/`controls_hospitalised')*100
...
local cases_`region'_pc = (`cases_`region''/`cases_hospitalised')*100
qui safecount if region==`region' & case==0
local controls_`region' = round(r(N),5)
local controls_`region'_pc = (`controls_`region''/`controls_hospitalised')*100
...
local cases_urban_pc = (`cases_urban'/`cases_hospitalised')*100
qui safecount if urban==1 & case==0
local controls_urban = round(r(N),5)
local controls_urban_pc = (`controls_urban'/`controls_hospitalised')*100
...
local cases_rural_pc = (`cases_rural'/`cases_hospitalised')*100
qui safecount if urban==0 & case==0
local controls_rural = round(r(N),5)
local controls_rural_pc = (`controls_rural'/`controls_hospitalised')*100
...
local cases_`bmi'_pc = (`cases_`bmi''/`cases_hospitalised')*100
qui safecount if bmi==`bmi' & case==0
local controls_`bmi' = round(r(N),5)
local controls_`bmi'_pc = (`controls_`bmi''/`controls_hospitalised')*100
...
local cases_pc = (`cases'/`cases_hospitalised')*100
qui safecount if bmi==. & case==0
local controls = round(r(N),5)
local controls_pc = (`controls'/`controls_hospitalised')*100
...
local cases_smoking_pc = (`cases_smoking'/`cases_hospitalised')*100
qui safecount if smoking==`smoking' & case==0
local controls_smoking = round(r(N),5)
local controls_smoking_pc = (`controls_smoking'/`controls_hospitalised')*100
...
local cases_`group'_pc = (`cases_`group''/`cases_hospitalised')*100
qui safecount if egfr_group==`group' & case==0
local controls_`group' = round(r(N),5)
local controls_`group'_pc = (`controls_`group''/`controls_hospitalised')*100
...
local cases_aki_baseline_pc = (`cases_aki_baseline'/`cases_hospitalised')*100
qui safecount if aki_baseline==1 & case==0
local controls_aki_baseline = round(r(N),5)
local controls_aki_baseline_pc = (`controls_aki_baseline'/`controls_hospitalised')*100
...
local cases_cardiovascular_pc = (`cases_cardiovascular'/`cases_hospitalised')*100
qui safecount if cardiovascular==1 & case==0
local controls_cardiovascular = round(r(N),5)
local controls_cardiovascular_pc = (`controls_cardiovascular'/`controls_hospitalised')*100
...
local cases_diabetes_pc = (`cases_diabetes'/`cases_hospitalised')*100
qui safecount if diabetes==1 & case==0
local controls_diabetes = round(r(N),5)
local controls_diabetes_pc = (`controls_diabetes'/`controls_hospitalised')*100
...
local cases_hypertension_pc = (`cases_hypertension'/`cases_hospitalised')*100
qui safecount if hypertension==1 & case==0
local controls_hypertension = round(r(N),5)
local controls_hypertension_pc = (`controls_hypertension'/`controls_hospitalised')*100
...
local cases_immunosuppressed_pc = (`cases_immunosuppressed'/`cases_hospitalised')*100
qui safecount if immunosuppressed==1 & case==0
local controls_immunosuppressed = round(r(N),5)
local controls_immunosuppressed_pc = (`controls_immunosuppressed'/`controls_hospitalised')*100
...
local cases_non_haem_cancer_pc = (`cases_non_haem_cancer'/`cases_hospitalised')*100
qui safecount if non_haem_cancer==1 & case==0
local controls_non_haem_cancer = round(r(N),5)
local controls_non_haem_cancer_pc = (`controls_non_haem_cancer'/`controls_hospitalised')*100
...
local cases_`vax'_pc = (`cases_`vax''/`cases_hospitalised')*100
qui safecount if covid_vax==`vax' & case==0
local controls_`vax' = round(r(N),5)
local controls_`vax'_pc = (`controls_`vax''/`controls_hospitalised')*100
...
local cases_`wave'_pc = (`cases_`wave''/`cases_hospitalised')*100
qui safecount if wave==`wave' & case==0
local controls_`wave' = round(r(N),5)
local controls_`wave'_pc = (`controls_`wave''/`controls_hospitalised')*100
file write tablecontent %9.0f (`cases_`wave'') (" (") %4.1f (`cases_`wave'_pc') (")") _tab %9.0f (`controls_`wave'') (" (") %4.1f (`controls_`wave'_pc') (")") _n
}
