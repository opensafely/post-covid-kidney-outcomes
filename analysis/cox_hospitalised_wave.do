...
cap log close
macro drop hr
log using ./logs/cox_hospitalised_wave.log, replace t

cap file close tablecontent
file open tablecontent using ./output/cox_hospitalised_wave.txt, write text replace
...
forvalues i=1/4 {
	...
	local event = r(N)
	qui su total_follow_up_`outcome' if wave==`i'
	local person_year = r(mean)/365.25
	local rate = 100000*(`event'/`person_year')
