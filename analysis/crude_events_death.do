...
cap log close
macro drop hr
log using ./logs/crude_events_death.log, replace t

cap file close tablecontent
file open tablecontent using ./output/crude_events_death.csv, write text replace
...
foreach x of local cohort {
use ./output/analysis_`x'.dta, clear
qui safecount if case==1 & death==1
local cases_events = round(r(N),5)
qui safecount if case==0 & death==1
...
foreach x of local cohort {
use ./output/analysis_`x'.dta, clear
qui safecount if agegroup==`age' & case==1 & death==1
local cases_events = round(r(N),5)
qui safecount if agegroup==`age' & case==0 & death==1
local controls_events = round(r(N),5)
file write tablecontent (`cases_events') _tab (`controls_events') _tab
}
file write tablecontent _n
...
foreach x of local cohort {
use ./output/analysis_`x'.dta, clear
qui safecount if sex==`sex' & case==1 & death==1
local cases_events = round(r(N),5)
qui safecount if sex==`sex' & case==0 & death==1
local controls_events = round(r(N),5)
file write tablecontent (`cases_events') _tab (`controls_events') _tab
}
file write tablecontent _n
...
foreach x of local cohort {
use ./output/analysis_`x'.dta, clear
qui safecount if imd==`imd' & case==1 & death==1
local cases_events = round(r(N),5)
qui safecount if imd==`imd' & case==0 & death==1
local controls_events = round(r(N),5)
file write tablecontent (`cases_events') _tab (`controls_events') _tab
}
file write tablecontent _n
...
foreach x of local cohort {
use ./output/analysis_`x'.dta, clear
qui safecount if ethnicity==`ethnicity' & case==1 & death==1
local cases_events = round(r(N),5)
qui safecount if ethnicity==`ethnicity' & case==0 & death==1
local controls_events = round(r(N),5)
file write tablecontent (`cases_events') _tab (`controls_events') _tab
}
file write tablecontent _n
...
foreach x of local cohort {
use ./output/analysis_`x'.dta, clear
qui safecount if ethnicity==. & case==1 & death==1
local cases_events = round(r(N),5)
qui safecount if ethnicity==. & case==0 & death==1
local controls_events = round(r(N),5)
file write tablecontent (`cases_events') _tab (`controls_events') _tab
}
file write tablecontent _n
...
foreach x of local cohort {
use ./output/analysis_`x'.dta, clear
qui safecount if urban==1 & case==1 & death==1
local cases_events = round(r(N),5)
qui safecount if urban==1 & case==0 & death==1
local controls_events = round(r(N),5)
file write tablecontent (`cases_events') _tab (`controls_events') _tab
}
file write tablecontent _n
...
foreach x of local cohort {
use ./output/analysis_`x'.dta, clear
qui safecount if bmi==`bmi' & case==1 & death==1
local cases_events = round(r(N),5)
qui safecount if bmi==`bmi' & case==0 & death==1
local controls_events = round(r(N),5)
file write tablecontent (`cases_events') _tab (`controls_events') _tab
}
file write tablecontent _n
...
foreach x of local cohort {
use ./output/analysis_`x'.dta, clear
qui safecount if bmi==. & case==1 & death==1
local cases_events = round(r(N),5)
qui safecount if bmi==. & case==0 & death==1
local controls_events = round(r(N),5)
file write tablecontent (`cases_events') _tab (`controls_events') _tab
}
file write tablecontent _n
...
foreach x of local cohort {
use ./output/analysis_`x'.dta, clear
qui safecount if smoking==`smoking' & case==1 & death==1
local cases_events = round(r(N),5)
qui safecount if smoking==`smoking' & case==0 & death==1
local controls_events = round(r(N),5)
file write tablecontent (`cases_events') _tab (`controls_events') _tab
}
file write tablecontent _n
...
foreach x of local cohort {
use ./output/analysis_`x'.dta, clear
qui safecount if smoking==. & case==1 & death==1
local cases_events = round(r(N),5)
qui safecount if smoking==. & case==0 & death==1
local controls_events = round(r(N),5)
file write tablecontent (`cases_events') _tab (`controls_events') _tab
}
file write tablecontent _n
...
foreach x of local cohort {
use ./output/analysis_`x'.dta, clear
qui safecount if egfr_group==`group' & case==1 & death==1
local cases_events = round(r(N),5)
qui safecount if egfr_group==`group' & case==0 & death==1
local controls_events = round(r(N),5)
file write tablecontent (`cases_events') _tab (`controls_events') _tab
}
file write tablecontent _n
...
foreach x of local cohort {
use ./output/analysis_`x'.dta, clear
qui safecount if aki_baseline==`aki' & case==1 & death==1
local cases_events = round(r(N),5)
qui safecount if aki_baseline==`aki' & case==0 & death==1
local controls_events = round(r(N),5)
file write tablecontent (`cases_events') _tab (`controls_events') _tab
}
file write tablecontent _n
...
foreach x of local cohort {
use ./output/analysis_`x'.dta, clear
qui safecount if cardiovascular==1 & case==1 & death==1
local cases_events = round(r(N),5)
qui safecount if cardiovascular==1 & case==0 & death==1
local controls_events = round(r(N),5)
file write tablecontent (`cases_events') _tab (`controls_events') _tab
}
file write tablecontent _n
...
foreach x of local cohort {
use ./output/analysis_`x'.dta, clear
qui safecount if diabetes==`diabetes' & case==1 & death==1
local cases_events = round(r(N),5)
qui safecount if diabetes==`diabetes' & case==0 & death==1
local controls_events = round(r(N),5)
file write tablecontent (`cases_events') _tab (`controls_events') _tab
}
file write tablecontent _n
...
foreach x of local cohort {
use ./output/analysis_`x'.dta, clear
qui safecount if hypertension==1 & case==1 & death==1
local cases_events = round(r(N),5)
qui safecount if hypertension==1 & case==0 & death==1
local controls_events = round(r(N),5)
file write tablecontent (`cases_events') _tab (`controls_events') _tab
}
file write tablecontent _n
...
foreach x of local cohort {
use ./output/analysis_`x'.dta, clear
qui safecount if immunosuppressed==1 & case==1 & death==1
local cases_events = round(r(N),5)
qui safecount if immunosuppressed==1 & case==0 & death==1
local controls_events = round(r(N),5)
file write tablecontent (`cases_events') _tab (`controls_events') _tab
}
file write tablecontent _n
...
foreach x of local cohort {
use ./output/analysis_`x'.dta, clear
qui safecount if non_haem_cancer==1 & case==1 & death==1
local cases_events = round(r(N),5)
qui safecount if non_haem_cancer==1 & case==0 & death==1
local controls_events = round(r(N),5)
file write tablecontent (`cases_events') _tab (`controls_events') _tab
}
file write tablecontent _n
...
foreach x of local cohort {
use ./output/analysis_`x'.dta, clear
qui safecount if gp_consults==`group' & case==1 & death==1
local cases_events = round(r(N),5)
qui safecount if gp_consults==`group' & case==0 & death==1
local controls_events = round(r(N),5)
file write tablecontent (`cases_events') _tab (`controls_events') _tab
}
file write tablecontent _n
...
foreach x of local cohort {
use ./output/analysis_`x'.dta, clear
qui safecount if admissions==`group' & case==1 & death==1
local cases_events = round(r(N),5)
qui safecount if admissions==`group' & case==0 & death==1
local controls_events = round(r(N),5)
file write tablecontent (`cases_events') _tab (`controls_events') _tab
}
file write tablecontent _n
...
foreach x of local cohort {
use ./output/analysis_`x'.dta, clear
qui safecount if covid_vax==`group' & case==1 & death==1
local cases_events = round(r(N),5)
qui safecount if covid_vax==`group' & case==0 & death==1
local controls_events = round(r(N),5)
file write tablecontent (`cases_events') _tab (`controls_events') _tab
}
file write tablecontent _n
