...
* Open a log file
cap log close
log using ./logs/split_stps_community_only.log, replace t


*(1)=========Split cases into separate stp files============
import delimited ./output/covid_community_matching.csv, clear
...
foreach i of numlist 5/9 {
	preserve
		...
		capture noisily export delimited using "./output/input_covid_community_matching_stp`i'.csv", replace
		count
	restore
}
...
foreach i of numlist 5/9  {
	preserve
		capture noisily keep if stp=="E5400000`i'"
		capture noisily export delimited using "./output/input_contemporary_matching_stp`i'.csv", replace
		count
	restore
}
