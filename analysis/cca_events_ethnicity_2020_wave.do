...
cap log close
macro drop hr
log using ./logs/cca_events_ethnicity_2020_wave.log, replace t

cap file close tablecontent
file open tablecontent using ./output/cca_events_ethnicity_2020_wave.csv, write text replace
file write tablecontent ("ethnicity") _tab ("outcome") _tab ("wave") _tab ("COVID-19 cohort") _tab ("Matched cohort (contemporary)") _n
...
qui safecount if ethnicity==`ethnicity' & wave==`i' & _d==1 & _st==1 & case==1
local cases_events = round(r(N),5)
qui safecount if ethnicity==`ethnicity' & wave==`i' & _d==1 & _st==1 & case==0
local controls_events = round(r(N),5)
file write tablecontent ("`label_`ethnicity''") _tab ("``out'_lab'") _tab ("`wave_`i''") _tab (`cases_events') _tab (`controls_events') _n
}
}
}
