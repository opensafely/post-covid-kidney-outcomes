...
cap log close
macro drop hr
log using ./logs/cox_models_covid_aki_egfr_half.log, replace t
cap file close tablecontent
file open tablecontent using ./output/cox_models_covid_aki_egfr_half.csv, write text replace
...
qui stcox i.covid_aki i.agegroup i.ethnicity i.imd i.urban i.bmi i.smoking i.ckd_stage i.aki_baseline i.cardiovascular i.diabetes i.hypertension i.immunosuppressed i.non_haem_cancer i.gp_consults i.admissions, strata(set_id)
est store a
qui stcox i.covid_aki##i.agegroup i.ethnicity i.imd i.urban i.bmi i.smoking i.ckd_stage i.aki_baseline i.cardiovascular i.diabetes i.hypertension i.immunosuppressed i.non_haem_cancer i.gp_consults i.admissions, strata(set_id)
est store b
qui lrtest b a
...
forvalues j=1/3 {
forvalues i=1/6 {
lincom `j'.covid_aki + `j'.covid_aki#`i'.agegroup, eform
...
forvalues i=1/6 {
bysort covid_aki: egen total_follow_up_`i' = total(_t) if agegroup==`i'
forvalues j=1/3 {
qui su total_follow_up_`i' if covid_aki==`j' & agegroup==`i'
local cases_multip = 100000 / r(mean)
qui safecount if covid_aki==`j' & _d==1 & _st==1 & agegroup==`i'
...
use ./output/analysis_`x'.dta, clear
stset exit_date_egfr_half, fail(egfr_half_date) origin(index_date_egfr_half) id(unique) scale(365.25)
qui stcox i.covid_aki i.sex i.ethnicity i.imd i.urban i.bmi i.smoking i.ckd_stage i.aki_baseline i.cardiovascular i.diabetes i.hypertension i.immunosuppressed i.non_haem_cancer i.gp_consults i.admissions, strata(set_id)
est store a
qui stcox i.covid_aki##i.sex i.ethnicity i.imd i.urban i.bmi i.smoking i.ckd_stage i.aki_baseline i.cardiovascular i.diabetes i.hypertension i.immunosuppressed i.non_haem_cancer i.gp_consults i.admissions, strata(set_id)
est store b
qui lrtest b a
...
forvalues j=1/3 {
forvalues i=0/1 {
lincom `j'.covid_aki + `j'.covid_aki#`i'.sex, eform
local int_`j'`i'b_`x' = r(estimate)
local int_`j'`i'll_`x' = r(lb)
local int_`j'`i'ul_`x' = r(ub)
}
...
local cases_multip = 100000 / r(mean)
qui safecount if covid_aki==`j' & _d==1 & _st==1 & sex==`i'
local cases_events = round(r(N),5)
local cases_rate`j'`i'_`x' = (`cases_events' * `cases_multip')
local cases_ef = exp(1.96/(sqrt(`cases_events')))
local cases_ul`j'`i'_`x' = `cases_rate`j'`i'_`x'' * `cases_ef'
...
use ./output/analysis_`x'.dta, clear
stset exit_date_egfr_half, fail(egfr_half_date) origin(index_date_egfr_half) id(unique) scale(365.25)
qui stcox i.covid_aki i.ethnicity i.imd i.urban i.bmi i.smoking i.ckd_stage i.aki_baseline i.cardiovascular i.diabetes i.hypertension i.immunosuppressed i.non_haem_cancer i.gp_consults i.admissions, strata(set_id)
est store a
qui stcox i.covid_aki##i.ethnicity i.imd i.urban i.bmi i.smoking i.ckd_stage i.aki_baseline i.cardiovascular i.diabetes i.hypertension i.immunosuppressed i.non_haem_cancer i.gp_consults i.admissions, strata(set_id)
est store b
qui lrtest b a
...
forvalues j=1/3 {
forvalues i=1/5 {
lincom `j'.covid_aki + `j'.covid_aki#`i'.ethnicity, eform
local int_`j'`i'b_`x' = r(estimate)
local int_`j'`i'll_`x' = r(lb)
local int_`j'`i'ul_`x' = r(ub)
}
...
local cases_multip = 100000 / r(mean)
qui safecount if covid_aki==`j' & _d==1 & _st==1 & ethnicity==`i'
local cases_events = round(r(N),5)
local cases_rate`j'`i'_`x' = (`cases_events' * `cases_multip')
local cases_ef = exp(1.96/(sqrt(`cases_events')))
local cases_ul`j'`i'_`x' = `cases_rate`j'`i'_`x'' * `cases_ef'
...
qui stcox i.covid_aki i.ethnicity i.imd i.urban i.bmi i.smoking i.ckd_stage i.aki_baseline i.cardiovascular i.diabetes i.hypertension i.immunosuppressed i.non_haem_cancer i.gp_consults i.admissions, strata(set_id)
est store a
qui stcox i.covid_aki##i.imd i.ethnicity i.urban i.bmi i.smoking i.ckd_stage i.aki_baseline i.cardiovascular i.diabetes i.hypertension i.immunosuppressed i.non_haem_cancer i.gp_consults i.admissions, strata(set_id)
est store b
qui lrtest b a
local p_`x' = r(p)
*Obtain stratum specific HRs
forvalues j=1/3 {
forvalues i=1/5 {
lincom `j'.covid_aki + `j'.covid_aki#`i'.imd, eform
local int_`j'`i'b_`x' = r(estimate)
local int_`j'`i'll_`x' = r(lb)
local int_`j'`i'ul_`x' = r(ub)
}
...
local cases_multip = 100000 / r(mean)
qui safecount if covid_aki==`j' & _d==1 & _st==1 & imd==`i'
local cases_events = round(r(N),5)
local cases_rate`j'`i'_`x' = (`cases_events' * `cases_multip')
local cases_ef = exp(1.96/(sqrt(`cases_events')))
local cases_ul`j'`i'_`x' = `cases_rate`j'`i'_`x'' * `cases_ef'
...
qui stcox i.covid_aki i.ethnicity i.imd i.urban i.bmi i.smoking i.ckd_stage i.aki_baseline i.cardiovascular i.diabetes i.hypertension i.immunosuppressed i.non_haem_cancer i.gp_consults i.admissions, strata(set_id)
est store a
qui stcox i.covid_aki##i.diabetes i.ethnicity i.imd i.urban i.bmi i.smoking i.ckd_stage i.aki_baseline i.cardiovascular i.hypertension i.immunosuppressed i.non_haem_cancer i.gp_consults i.admissions, strata(set_id)
est store b
qui lrtest b a
local p_`x' = r(p)
*Obtain stratum specific HRs
forvalues j=1/3 {
forvalues i=0/1 {
lincom `j'.covid_aki + `j'.covid_aki#`i'.diabetes, eform
local int_`j'`i'b_`x' = r(estimate)
local int_`j'`i'll_`x' = r(lb)
local int_`j'`i'ul_`x' = r(ub)
}
...
local cases_multip = 100000 / r(mean)
qui safecount if covid_aki==`j' & _d==1 & _st==1 & diabetes==`i'
local cases_events = round(r(N),5)
local cases_rate`j'`i'_`x' = (`cases_events' * `cases_multip')
local cases_ef = exp(1.96/(sqrt(`cases_events')))
local cases_ul`j'`i'_`x' = `cases_rate`j'`i'_`x'' * `cases_ef'
...
use ./output/analysis_`x'.dta, clear
stset exit_date_egfr_half, fail(egfr_half_date) origin(index_date_egfr_half) id(unique) scale(365.25)
qui stcox i.covid_aki i.ethnicity i.imd i.urban i.bmi i.smoking i.egfr_group i.aki_baseline i.cardiovascular i.diabetes i.hypertension i.immunosuppressed i.non_haem_cancer i.gp_consults i.admissions, strata(set_id)
est store a
qui stcox i.covid_aki##i.egfr_group i.imd i.ethnicity i.urban i.bmi i.smoking i.aki_baseline i.cardiovascular i.diabetes i.hypertension i.immunosuppressed i.non_haem_cancer i.gp_consults i.admissions, strata(set_id)
est store b
qui lrtest b a
local p_`x' = r(p)
*Obtain stratum specific HRs
forvalues j=1/3 {
forvalues i=1/7 {
lincom `j'.covid_aki + `j'.covid_aki#`i'.egfr_group, eform
local int_`j'`i'b_`x' = r(estimate)
local int_`j'`i'll_`x' = r(lb)
local int_`j'`i'ul_`x' = r(ub)
}
...
local cases_multip = 100000 / r(mean)
qui safecount if covid_aki==`j' & _d==1 & _st==1 & egfr_group==`i'
local cases_events = round(r(N),5)
local cases_rate`j'`i'_`x' = (`cases_events' * `cases_multip')
local cases_ef = exp(1.96/(sqrt(`cases_events')))
local cases_ul`j'`i'_`x' = `cases_rate`j'`i'_`x'' * `cases_ef'
...
qui stcox i.covid_aki i.ethnicity i.imd i.urban i.bmi i.smoking i.ckd_stage i.aki_baseline i.cardiovascular i.diabetes i.hypertension i.immunosuppressed i.non_haem_cancer i.gp_consults i.admissions, strata(set_id)
est store a
qui stcox i.covid_aki##i.aki_baseline i.diabetes i.ethnicity i.imd i.urban i.bmi i.smoking i.ckd_stage i.cardiovascular i.hypertension i.immunosuppressed i.non_haem_cancer i.gp_consults i.admissions, strata(set_id)
est store b
qui lrtest b a
local p_`x' = r(p)
*Obtain stratum specific HRs
forvalues j=1/3 {
forvalues i=0/1 {
lincom `j'.covid_aki + `j'.covid_aki#`i'.aki_baseline, eform
local int_`j'`i'b_`x' = r(estimate)
local int_`j'`i'll_`x' = r(lb)
local int_`j'`i'ul_`x' = r(ub)
}
...
local cases_multip = 100000 / r(mean)
qui safecount if covid_aki==`j' & _d==1 & _st==1 & aki_baseline==`i'
local cases_events = round(r(N),5)
local cases_rate`j'`i'_`x' = (`cases_events' * `cases_multip')
local cases_ef = exp(1.96/(sqrt(`cases_events')))
local cases_ul`j'`i'_`x' = `cases_rate`j'`i'_`x'' * `cases_ef'
...
*Obtain p-values for interaction
stset exit_date_egfr_half, fail(egfr_half_date) origin(index_date_egfr_half) id(unique) scale(365.25)
qui stcox i.covid_aki i.ethnicity i.imd i.urban i.bmi i.smoking i.ckd_stage i.aki_baseline i.cardiovascular i.diabetes i.hypertension i.immunosuppressed i.non_haem_cancer i.gp_consults i.admissions i.wave i.covid_vax, strata(set_id)
est store a
qui stcox i.covid_aki##i.wave i.imd i.ethnicity i.urban i.bmi i.smoking i.ckd_stage i.aki_baseline i.cardiovascular i.diabetes i.hypertension i.immunosuppressed i.non_haem_cancer i.gp_consults i.admissions i.covid_vax, strata(set_id)
est store b
qui lrtest b a
local p = r(p)
*Obtain stratum specific HRs
forvalues j=1/3 {
forvalues i=1/4 {
lincom `j'.covid_aki + `j'.covid_aki#`i'.wave, eform
...
local cases_multip = 100000 / r(mean)
qui safecount if covid_aki==`j' & _d==1 & _st==1 & wave==`i'
local cases_events = round(r(N),5)
...
qui stcox i.covid_aki i.ethnicity i.imd i.urban i.bmi i.smoking i.ckd_stage i.aki_baseline i.cardiovascular i.diabetes i.hypertension i.immunosuppressed i.non_haem_cancer i.gp_consults i.admissions i.wave i.covid_vax, strata(set_id)
est store a
qui stcox i.covid_aki##i.covid_vax i.wave i.imd i.ethnicity i.urban i.bmi i.smoking i.ckd_stage i.aki_baseline i.cardiovascular i.diabetes i.hypertension i.immunosuppressed i.non_haem_cancer i.gp_consults i.admissions, strata(set_id)
est store b
qui lrtest b a
local p = r(p)
*Obtain stratum specific HRs
forvalues j=1/3 {
forvalues i=1/5 {
lincom `j'.covid_aki + `j'.covid_aki#`i'.covid_vax, eform
local int_`j'`i'b = r(estimate)
local int_`j'`i'll = r(lb)
local int_`j'`i'ul = r(ub)
}
...
local cases_multip = 100000 / r(mean)
qui safecount if covid_aki==`j' & _d==1 & _st==1 & covid_vax==`i'
local cases_events = round(r(N),5)
local cases_rate`j'`i' = (`cases_events' * `cases_multip')
local cases_ef = exp(1.96/(sqrt(`cases_events')))
local cases_ul`j'`i' = `cases_rate`j'`i'' * `cases_ef'
