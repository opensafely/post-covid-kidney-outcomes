...
stcox i.covid_vax i.sex i.ethnicity i.imd i.urban i.region i.bmi i.cardiovascular i.diabetes i.hypertension i.immunosuppressed i.non_haem_cancer i.smoking age1 age2 age3, vce(cluster practice_id) strata(set_id)		
if _rc==0{
estimates
...
forvalues vax=1/5 {
	...
	local person_year = r(mean)
	local rate = 100000*(`r_event'/`person_year')
	file write tablecontent _tab ("`lab`vax''") _tab _tab _tab (`r_denominator') _tab _tab (`r_event') _tab %10.0f (`person_year') _tab _tab %3.2f (`rate ') _tab _tab _tab
